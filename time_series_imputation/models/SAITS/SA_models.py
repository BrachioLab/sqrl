...
def result_processing(results, reconstruction_loss_weight, imputation_loss_weight):
    ...
    results['reconstruction_loss'] = results['reconstruction_loss'] * reconstruction_loss_weight
    results['imputation_loss'] = results['imputation_loss'] * imputation_loss_weight
    # if args.MIT:
    total_loss += results['imputation_loss']
    # if args.ORT:
    total_loss += results['reconstruction_loss']
    return total_loss
...
class SAITS(nn.Module):
    ...
    def forward(self, subsampled_data, subsampled_tp, subsampled_mask, reconstruction_loss_weight, imputation_loss_weight):
        X, masks = subsampled_data, subsampled_mask
        reconstruction_loss = 0
        imputed_data, [X_tilde_1, X_tilde_2, X_tilde_3] = self.impute(subsampled_data, subsampled_mask)
        ...
        # if (self.MIT or stage == 'val') and stage != 'test':
        #     # have to cal imputation loss in the val stage; no need to cal imputation loss here in the test stage
        #     imputation_MAE = masked_mae_cal(X_tilde_3, subsampled_data, gt_mask)
        # else:
        imputation_MAE = 0
        ...
        loss = result_processing(results, reconstruction_loss_weight, imputation_loss_weight)
        
        return loss
    
    def evaluate(self, subsampled_data, subsampled_tp, subsampled_mask, gt_mask):
        imputed_data, [X_tilde_1, X_tilde_2, X_tilde_3] = self.impute(subsampled_data, gt_mask)
        return imputed_data
