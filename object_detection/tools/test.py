...
from mmdet.apis import multi_gpu_test, single_gpu_test, output_gt_bbox_for_comparison
...
def parse_args():
    ...
    parser.add_argument('--out_dir', default=None, help='output result file in pickle format')
    parser.add_argument(
        '--fuse_conv_bn',
        action='store_true',
    ...
    # rule_path
    parser.add_argument('--rule_path', type=int, default=0)
    parser.add_argument(
        '--full_model',
        action='store_true',
...
def main():
    ...
    data_loader = build_dataloader(
        ...
        dist=distributed,
        shuffle=False)
    output_gt_bbox_for_comparison(args.out_dir, data_loader)
    ...
    if not distributed:
        ...
        outputs = single_gpu_test(model, data_loader, args.show, args.eval if args.eval[0] == 'panoptic' else None, save_dir=args.out_dir)
    else:
