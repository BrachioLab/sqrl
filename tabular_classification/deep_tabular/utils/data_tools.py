...
def get_data(path, dataset_id, source, task, datasplit=[.65, .15, .2]):
    """
    ...
    data_train, data_val, data_test = split_df_attrs_all(data_train, data_val, data_test, path, numerical_columns, categorical_columns)
    return numerical_data, categorical_data, targets, info, full_cat_data_for_encoder, {"train_df": data_train, "val_df": data_val, "test_df": data_test}

def split_df_attrs(df, split_mappings, numerical_columns, categorical_columns):
    for col in numerical_columns:
        percentile_ls = split_mappings[col]
        for idx in range(len(percentile_ls)+1):
            df[col + "_" + str(idx)] = np.zeros(len(df))
            if idx == 0:
                
                df.loc[df[col] <= percentile_ls[0], col + "_" + str(idx)] = 1
            elif idx < len(percentile_ls):
                df.loc[(df[col] > percentile_ls[idx-1]) & (df[col] <= percentile_ls[idx]), col + "_" + str(idx)] = 1
            else:
                df.loc[(df[col] > percentile_ls[idx-1]), col + "_" + str(idx)] = 1
    
    for col in categorical_columns:
        unique_val_ls = split_mappings[col]
        for val in unique_val_ls:
            df[col + "_" + str(val)] = np.zeros(len(df))
            df.loc[df[col] == val, col + "_" + str(val)] = 1

    return df

def split_df_attrs_all(train_df, valid_df, test_df, data_path, numerical_columns, categorical_columns):

    with open(os.path.join(data_path, "dataframe_split.json")) as f:
        split_mappings = json.load(f)

    
    train_df = split_df_attrs(train_df, split_mappings, numerical_columns, categorical_columns)
    valid_df = split_df_attrs(valid_df, split_mappings, numerical_columns, categorical_columns)
    test_df = split_df_attrs(test_df, split_mappings, numerical_columns, categorical_columns)
    return train_df, valid_df, test_df

def get_multilabel_data(ds_id, source, task, path=None):
    """
    ...
    if os.path.exists(os.path.join(path, 'N.csv')):
        X_full_num = pd.read_csv(os.path.join(path, 'N.csv'))
        numerical_columns = list(X_full_num.columns)
    elif os.path.exists(f'../../../data/{ds_id}/N.csv'):
        X_full_num = pd.read_csv(f'../../../data/{ds_id}/N.csv')
        numerical_columns = list(X_full_num.columns)
    else:
        X_full_num = pd.DataFrame()
    ...
    if os.path.exists(os.path.join(path, 'C.csv')):
        X_full_cat = pd.read_csv(os.path.join(path, 'C.csv'))
        categorical_columns = list(X_full_cat.columns)
    elif os.path.exists(f'../../../data/{ds_id}/C.csv'):
        X_full_cat = pd.read_csv(f'../../../data/{ds_id}/C.csv')
        categorical_columns = list(X_full_cat.columns)
    else:
        X_full_cat = pd.DataFrame()
    ...
    X_full = pd.concat([X_full_num, X_full_cat], axis = 1)
    if path is not None:
        y_full = pd.read_csv(os.path.join(path, 'y.csv'))
    else:
        y_full = pd.read_csv(f'../../../data/{ds_id}/y.csv')
    ...
    # X_train, X_test, y_train, y_test = train_test_split(X_full, y_full, test_size=0.4, random_state=1)
    ...
    if len(y_train_np.shape) > 1:
        info['n_classes'] = y_train_np.shape[1]
    else:
        info['n_classes'] = 1
    ...
    targets = {'train': y_train_np, 'val': y_val_np, 'test': y_test_np}
    print('\n Train size:{} Val size:{} Test size:{}'.format(len(y_train_np), len(y_val_np), len(y_test_np)))
    ...
    X_train, X_val, X_test = split_df_attrs_all(X_train, X_val, X_test, path, numerical_columns, categorical_columns)

    return numerical_data, categorical_data, targets, info, full_cat_data_for_encoder, {"train_df": pd.concat([X_train, y_train], axis=1), "val_df": pd.concat([X_val, y_val], axis=1), "test_df": pd.concat([X_test, y_test], axis=1)}
