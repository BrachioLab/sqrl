...
class Norm(nn.Module):
    ...
    def __init__(self, model,reset_stats=False, no_stats=True):
        super().__init__()
        self.model = model
        self.model = configure_model(model, reset_stats,
                                     no_stats)
...
def configure_model(model, reset_stats, no_stats):
...
def configure_model2(model, eps, momentum, reset_stats, no_stats):
    """Configure model for adaptation by test-time normalization."""
    for m in model.modules():
        if isinstance(m, nn.BatchNorm2d):
            # use batch-wise statistics in forward
